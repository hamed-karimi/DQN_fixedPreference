...
class MetaController:
    ...
    def __init__(self, params, pre_trained_weights_path=''):
        ...
        if self.gamma_cascade:
            ...
            self.gammas = [self.max_gamma for _ in range(params.MAX_STEP_NUM)]
    ...
    def get_goal_map(self, environment, agent, episode, epsilon=None):
        ...
        if e < epsilon:  # random (goal or stay)
            ...
            with torch.no_grad():
                ...
                preference = agent.preference.to(self.device)
                output_values = self.policy_net(env_map, preference)
